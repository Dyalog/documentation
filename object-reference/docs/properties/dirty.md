...
However, once a page has content, setting Dirty to `0` does not stop that page from being printed. The operating system does not allow an individual page to be cancelled; the only way to prevent a page from being printed is to cancel the entire document.
