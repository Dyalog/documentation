...
If a class is to be based upon a specific .NET class, then the name of that .NET class must be specified as the base class in the [`:Class`](../../programming-reference-guide/defined-functions-and-operators/traditional-functions-and-operators/control-structures/class.md) statement, and the [`:Using`](../../programming-reference-guide/defined-functions-and-operators/traditional-functions-and-operators/control-structures/using.md) statements must correctly locate the base class. Otherwise, the class is assumed to be based on <code class="language-nonAPL">System.Object</code>. If you use any .NET types within your class, you must ensure that these too are located by `:Using`.
...
If you want to repeat the most recent export after making changes to the class, you can click on the icon to the right of the save icon on the WS button bar at the top of the session. The workspace is not saved when you do an export, so if you want the export options to be remembered you must [`)SAVE`](../../language-reference-guide/system-commands/save.md) the workspace after you have exported it.
