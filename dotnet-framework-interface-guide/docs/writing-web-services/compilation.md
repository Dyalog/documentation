...
The following script defines a class, instances of which would run using [`⎕ML←2`](../../language-reference-guide/system-functions/ml.md), containing a single function `FOO` and a variable `X`.
```apl
:Class MyClas
   ⎕ML←2
   X←10
...
If your script contains a [`⎕CY`](../../language-reference-guide/system-functions/cy.md) statement, it will be executed by the Dyalog .NET Compiler when establishing the class. This can be used to import functions from other workspaces and remove the need to include them in the **.asmx** file.
