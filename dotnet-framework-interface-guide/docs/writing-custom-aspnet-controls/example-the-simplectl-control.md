...
The `Render` function supersedes the <code class="language-nonAPL">Render</code> method that `SimpleCtl` has inherited from its base class, <code class="language-nonAPL">System.Web.UI.Control</code>.
...
The SimpleCtl control can now be included in any .NET Framework web page from which **temp.dll** is accessible. The file **[DYALOG]\Samples\asp.net\temp\Simple.aspx** is one example (it is immaterial that it is written in Dyalog):
```nonAPL
...
![Output when including the SimpleCtl control in a .NET Framework web page](../img/s-example-simplectl-control.png)
