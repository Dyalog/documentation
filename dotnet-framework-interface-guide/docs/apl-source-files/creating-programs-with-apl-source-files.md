...
The code must be contained within [`:NameSpace`](../../programming-reference-guide/defined-functions-and-operators/traditional-functions-and-operators/control-structures/namespace.md) and `:EndNamespace` statements, and must define a [`⎕LX`](../../language-reference-guide/system-functions/lx.md) either in the code itself or as a parameter to the `dyalogc` command.
