...
- compile an APL source file into a native Microsoft Windows executable program (**.exe**), which can be [run as a stand-alone executable](../../windows-installation-and-configuration-guide/runtime-applications-and-components.md#stand-alone-and-bound-run-times). This program can be distributed (along with the Dyalog APL runtime DLL) as a packaged application, and does not require any of the additional support files and registry entries that are typically needed by the Dyalog run-time **dyalogrt.exe**. For more information, see the _Dyalog for Microsoft Windows Installation and Configuration Guide_.
