...
To enter Dyalog APL symbols into an APL Source file, you need the Dyalog Input Method Editor (IME) or other APL compatible keyboard. The [Dyalog IME can be configured](../../windows-ui-guide/ime-configuration.md) from the Dyalog Configuration dialog box. You can change the associated **.DIN** file or there are various other options. APL Source files can also be edited using Microsoft Word, although they must be saved as text files without any Word formatting. For more information, see the _Dyalog for Microsoft Windows Installation and Configuration Guide_.
