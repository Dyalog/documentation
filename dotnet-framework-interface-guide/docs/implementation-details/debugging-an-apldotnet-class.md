...
- If you build a .NET class from a workspace using the **File**/**Export** menu item, the value in the **Runtime application** checkbox specifies the Dyalog DLL version; if **Runtime application** is not checked then the .NET class will be bound to the full development version, and if **Runtime application** is checked then the .NET class will be bound to the run-time version.
...
- If execution in the Dyalog DLL is suspended, you cannot execute [`)CLEAR`](../../language-reference-guide/system-commands/clear.md) or [`)RESET`](../../language-reference-guide/system-commands/reset.md). You can execute `)OFF` or [`⎕OFF`](../../language-reference-guide/system-functions/off.md), but doing so will cause the client application to terminate. If you attempt to close the APL Session window, you will be warned that this will terminate the client application.
