...
The possible isolation modes (and their `DyalogIsolationMode` values) are:

- DyalogIsolationAssembly – each assembly has its own Dyalog DLL and workspace. This means that a new engine is started for each assembly containing APL classes.
- DyalogIsolationApplication – each web application/AppDomain has its own Dyalog DLL and workspace. This means that, when running in IIS, a new engine is started for each ASP.NET application. In this context, Microsoft Internet Information Services (IIS) is a single process, even though it might be hosting a large number of different web pages. Each ASP.NET application will run in a separate AppDomain, a mechanism used by .NET to provide isolation within an application. Other .NET applications might also be divided into different AppDomains.
- DyalogIsolationProcess – each process has its own Dyalog DLL and workspace (this is the default). This means that all classes and instances used by any IIS web page will be hosted in the same workspace and share a single copy of the interpreter.
- DyalogIsolationLocal – each assembly attempts to use the local bridge and interpreter libraries.
