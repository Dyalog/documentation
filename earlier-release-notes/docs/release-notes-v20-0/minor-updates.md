...
Two exit codes have been added. The interpreter returns 12 if the HMON (health monitor) library cannot be loaded, and 13 if that library is loaded but cannot be initialised. An application that returns its own exit code from `⎕OFF` should therefore use a value greater than 13.
