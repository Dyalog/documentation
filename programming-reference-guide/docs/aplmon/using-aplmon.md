...
Accumulated data is only actually written to the file when the method is called or when Dyalog terminates. The name of the file to which the results are written is returned as a [shy](../introduction/results.md) result. This mechanism allows you to log application results in separate files.
