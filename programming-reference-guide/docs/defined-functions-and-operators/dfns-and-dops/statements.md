...
- an *expression*, whose value can become the result of the dfn;
...
Statements are separated by the diamond (`⋄`) or by newlines. A dfn returns the value of the first statement it evaluates that is neither an assignment nor a guard, and evaluation stops there. When a guard's condition is true, its right-hand expression is evaluated and returned. If evaluation reaches the end of the dfn without producing such a value, the dfn returns no result.
