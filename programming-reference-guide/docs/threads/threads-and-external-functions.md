...
There is no point in specifying a `⎕NA` call to be multi-threaded, unless you want to execute other APL threads at the same time.
...
If you wish to run the same `⎕NA` call in separate APL threads at the same time, you must ensure that the DLL is *thread-safe*. Functions in DLLs which are not *thread-safe*, must be prevented from running concurrently by using the `:Hold` control structure. All the standard Windows API DLLs **are** *thread safe*.
