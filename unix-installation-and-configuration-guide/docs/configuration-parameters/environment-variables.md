...
|`DYALOG_SERIALFILE`|This parameter specifies the full path to the text file containing your Dyalog serial number. If unset, the default location is `$HOME/.dyalog/serial`. Setting it is useful in a multi-user environment, where a single serial number file can be shared by all users.|
