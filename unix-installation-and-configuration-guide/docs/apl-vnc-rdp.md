...
Due to the different ways that Microsoft Windows and Unix handle keyboards, it is not possible to use RDP or VNC or X-Windows from a Windows client to control a Dyalog APL session running under a Unix window manager. In particular, all of the X-Window clients that Dyalog is aware of do not  fully support xkb key mappings.
