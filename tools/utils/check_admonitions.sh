...
# tools/utils/check_admonitions.py; this wrapper selects which files to check.
#
# Usage:
#   tools/utils/check_admonitions.sh                 # gate: fail on non-compliant admonitions in files changed vs origin/main
#   tools/utils/check_admonitions.sh --changed REF   # ... changed vs REF (a branch or commit)
#   tools/utils/check_admonitions.sh --all           # report every non-compliant admonition in the whole monorepo
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "check_admonitions: unknown argument '$1'" >&2; exit 2 ;;
  esac
  shift
done

checker="$(cd "$(dirname "$0")" && pwd)/check_admonitions.py"
...
if [ "${#changed[@]}" -eq 0 ]; then
  echo "check_admonitions: no changed markdown files vs $base; nothing to check."
  exit 0
fi
