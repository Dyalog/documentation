...
A search path for the Input Translate Table, useful when configuring a run-time application. Directory paths use the operating system's conventions and are separated by `;` (Windows) or `:` (Unix).
...
<!-- REVIEW(default): reconcile the Microsoft Windows default (aplkeys sub-directory) with the Unix default (the Dyalog directory, else /usr/dyalog). -->
