...
!!! Info "Information"
    On Unix this parameter is set by the supplied startup script (`mapl`). To find the location of the Dyalog executable itself, read it from the process information (for example `/proc/<process-id>/` on Linux, or the output of `ps`) rather than from this parameter.
