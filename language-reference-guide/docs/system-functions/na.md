...
For `⎕NA` on non-Windows platforms (UNIX, Linux and macOS), see also [`⎕NA` under UNIX](../../../unix-installation-and-configuration-guide/quadna).
...
On non-Windows platforms, `⎕NA` supports both system-supplied and user-written shared libraries. Locating the system library (`libc`) and using the Dyalog-supplied shared library differ between platforms; for details and examples, see [`⎕NA` under UNIX](../../../unix-installation-and-configuration-guide/quadna).
