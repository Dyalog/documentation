...
If `Y` is `¯1`, `⎕TALLOC` releases a previously allocated range of tokens identified by the left-argument `X`. The result `R` is a [shy](../../../programming-reference-guide/introduction/results#shy-results) `⍬`.
