...
!!! unix "Dyalog on AIX"
    The performance of `⎕SHELL` on AIX can be improved using the [`DYALOG_SHELL_SUBPROCESS`](../../../dyalog-configuration-guide/configuration-parameters/dyalog-shell-subprocess) configuration parameter; this configuration parameter is active by default.
