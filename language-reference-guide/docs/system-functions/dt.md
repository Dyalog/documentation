...
When <code>X<sub>Y</sub></code> is a pattern, the corresponding character vectors in `Y` are matched against it and decoded into date-times. The behaviour when parsing datetimes is always well-defined and predictable, but it is beholden upon the user to create sane formatting patterns: there are some patterns which will match some texts and not others, and superficial testing does not always reveal this to be the case. The following additional rules apply to the matching.
...
- a 2-element vector of two character vectors, which apply to <code>X<sub>Y</sub></code> and <code>X<sub>R</sub></code> respectively (used when both are patterns).
