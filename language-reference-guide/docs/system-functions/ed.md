...
In this case, `⎕ED` terminates and returns a result only when the user explicitly closes all the windows for the named objects. The result contains the names of any objects that have been newly (re)fixed in the workspace as a result of the `⎕ED`, and has the same structure as `Y`.
