...
An attempt to update a component file that would cause it to exceed its [maximum size](fresize-dyadic.md) will fail with a `FILE FULL` error (21). `⎕FRESIZE` compacts the file. This process removes any gaps in the file caused by replacing a component with a shorter array. Any interrupt entered at the keyboard during the compaction is ignored. The file is compacted and its maximum size remains unchanged.
