...
# <span>Convert XML</span> `R←⎕XML Y`{{key}}

`⎕XML` converts an XML string into an APL array or converts an APL array into an XML string.

Options for `⎕XML` are specified using the _variant_ operator `⍠`.
...
There are 3 options, which are specified using the _variant_ operator `⍠`. The names are different and are case-sensitive; they must be spelled exactly as shown below.
