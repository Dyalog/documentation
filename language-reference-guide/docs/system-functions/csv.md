...
|ForceQuotes|A number specifying the degree to which quotes are applied around fields even if not strictly required. Possible values are:<ul><li>`0` – added only if required</li><li>`1` – added to all fields containing character data and to fields containing numeric data if required</li><li>`2` – added to all fields even if not required</li></ul>If ForceQuotes is a scalar, the value is applied to all columns; if it is a vector of values then each number is applied to the corresponding column.|`0`|
