...
A library, referred to here generically as a a Dynamic Link Library (DLL), is a collection of functions typically written in C (or C++) each of which can take arguments and return a result.

Instructional examples using `⎕NA` can be found in the supplied workspace `quadna`.

The DLL can be part of the standard operating system software, a library purchased from a third party supplier, or one that you have written yourself.
...
The DLL can be specified using a full pathname, file extension, and function type.
...
Alternatively, a full or relative pathname can be supplied in the usual way:
```apl
...
|Structure  |`{...}` |Structure.                                                                                             |
|Count      |`[int]` |Rather than explicitly declaring multiple adjacent occurrences of the same the count option can be used|
...
Note that while appending the array specifier '`[]`' is formally correct, because the presence of the special qualifier (`0` or `#`) *implies* an array, the '`[]`' can be omitted: `<0T`, `>0T`, `=#C`, and so on.

Note also that the 0 and # specifiers can be used with data of all types (excluding `A` and `Z`) and widths. For example, in the Classic Edition, `<0U2` can be useful for dealing with Unicode.

### Type

The data type of the argument can be one of the following characters and  can be specified in lower or upper case:
...
The type specifier can be followed by the width of the value in bytes. For example:
...
Arrays are specified by following the basic data type with `[n]` or `[]`, where `n` indicates the number of elements in the array. In the C declaration, the number of elements in an array can be specified explicitly at compile time, or determined dynamically at runtime. In the latter case, the size of the array is often passed along with the array, in a separate argument. In this case, `n`, the number of elements is omitted from the specification. Note that C deals only in scalars and rank 1 (vector) arrays.
```c
...
An appropriate `⎕NA` declaration would be:
```apl
      'bar'⎕NA'mydll.foo U <{I2  {I1[6]} F8}[]'
```

A call on the function with two arguments – a count followed by a vector of structures:
```apl
      pad←⊂6⍴0
      bar 4,⊂(3 pad 1.4)(1 pad 5.9 )(2 pad 6.5 )(0 pad 0)
```
...
If a definition includes multiple adjacent occurrences of the same item, the count syntax can be used rather than explicitly repeating the same definition.
...
A DLL function might or might not return a result, and can take zero or more arguments. This syntax is reflected in the coding of the right argument of `⎕NA`. However, notice that the corresponding associated APL function is a result-returning niladic (if it takes no arguments) or monadic function. It cannot be dyadic and it must *always* return a vector result – a null one if there is no output from the DLL function. See Result Vector section below. Examples of the various combinations are:
...
Under Windows, a DLL can associate an *ordinal number* with any of its functions. This number can then be used to call the function as an alternative to calling it by name. Using `⎕NA` to call by ordinal number uses the same syntax but with the function name replaced with its ordinal number. For example:
```apl
      'fn57'⎕NA'... mydll|57 ...'
```
...
Note that if you were to reserve fewer elements than the DLL function actually used, the DLL function would write beyond the end of the reserved array and might cause the interpreter to crash with a System Error (syserror 999 on Windows or SIGSEGV on UNIX, Linux or macOS).
...
```apl
      'myfun'⎕NA'mydll|fun I <I' ⋄ fun 42 42
```
...
```apl
      'myfun_null'⎕NA'mydll|fun I P' ⋄ myfun_null 42 0
```
...
To simplify writing portable code for both Editions, you can specify the character `*` instead of `A` or `W` at the end of a function name. This will be replaced by `A` in the Classic Edition and `W` in the Unicode Edition.
...
It is beyond the scope of this manual to list *all* the Microsoft definitions and their C primitive equivalents, and indeed, DLLs from sources other than Microsoft might well employ their own distinct naming conventions.

In general, you should consult the documentation that accompanies the DLL in order to convert typedefs to primitive C types and thence to `⎕NA` declarations. The documentation might well refer you to the 'include' files which are part of the Software Development Kit, and in which the types are defined.
...
1. `LPSTR` is a pointer to a null-terminated string. The definition does not indicate whether this is input or output, so the safest coding would be `=0T[]` (providing the vector you supply for input is long enough to accommodate the result). You might be able to improve simplicity or performance if the documentation indicates that the pointer is 'input only' (<`0T[]`) or 'output only' (>`0T[]`). See Direction above.
